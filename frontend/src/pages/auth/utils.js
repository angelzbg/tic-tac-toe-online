...
export const useAuthForm = (initFieldsArray = () => []) => {
  ...
  const onChange = ({ target: { value, name } }) =>
    dispatch({ type: "changeValue", payload: { value, name } });
  ...
  const isValidForm = () => {
    ...
    state.fields.forEach((field, i) => {
      ...
      if (
        field?.valid?.field &&
        state.fields.find(({ name }) => name === field.valid.field).value !==
          field.value
      ) {
        errors[i] = true;
        return;
  ...
  return { state, onChange, isValidForm, getFieldsBody, setNetworkError };
};
